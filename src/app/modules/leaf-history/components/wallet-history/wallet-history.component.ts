...
export class WalletHistoryComponent implements OnInit {
  ...
  GetPaymentData() {
    ...
    let bodyData: IGetPayment = {
      ...
      ClientCategory: this.WalletHistoryForm.value.CategoryName ?? '',
      ClientId: this.WalletHistoryForm.value.ClientId ?? 0,
      PaymentTypeId: this.WalletHistoryForm.value.PaymentTypeId ?? 0,
      CreatedBy: this.loginDetails.RoleName != 'Admin' ? this.loginDetails.UserId : this.WalletHistoryForm.value.UserId,
  ...
  GetWalletHistory() {
    ...
    let bodyData: IGetWalletHistory = {
      ...
      ToDate: formatDate(this.WalletHistoryForm.value.toDate, 'yyyy-MM-dd', 'en-US'),
      TenantId: this.loginDetails.TenantId,
      UserId: this.WalletHistoryForm.value.UserId ?? 0,
  ...
  selectClient(client: any) {
    if (client == '') {
      this.WalletHistoryForm.controls['ClientId'].reset();
    }

    this.WalletHistoryForm.controls['ClientId'].setValue(client?.ClientId);
  }
  ...
  fromDateChange(event: MatDatepickerInputEvent<Date>): void {
    //  this.WalletHistoryForm.controls['toDate'].setValue(null);
